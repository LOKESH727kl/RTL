...
module top_cxs_pktzr_depktzr_tx(
	...
	logic tx_header_reg_vld;			// High when header register data is valid
	
	logic tx_cxs_activeack_d;			//Delayed version of CXS Active Ack signal
	
	logic tx_header_det;				// High when a header is detected in the stream
    ...
    logic [255:0] tx_header;    		// 256-bit header 
	
	logic [255:0] tx_header_reg;		// 256-bit header register to store header
...
always_ff @(posedge tx_cxs_clk or negedge tx_cxs_rst_n)
  ...
  else					{data_buf_valid,data_buf} <= (!data_buf_en)? 257'h0:
                                                     (data_buf_en) ? {tx_cxs_valid,tx_cxs_data[255:0]} : {data_buf_valid,data_buf};	
													 
always_ff @(posedge tx_cxs_clk or negedge tx_cxs_rst_n)
  if (!tx_cxs_rst_n)				data_buf_en <= 1'b0;
  else if ((! tx_cxs_activereq))	data_buf_en <= 1'b0;
  else if (tx_cxs_activeack) 		data_buf_en <= 1'b1;	
  
always_ff @(posedge tx_cxs_clk or negedge tx_cxs_rst_n)
  if (!tx_cxs_rst_n)	cxs_tx_max_pkt_perflit <= 2'h0;
  else 					cxs_tx_max_pkt_perflit <= (tx_cxs_activeack) ? MAX_PKT_PERFLIT : 2'h0;
  
always_ff @(posedge tx_cxs_clk or negedge tx_cxs_rst_n)
  if (!tx_cxs_rst_n)	cxs_tx_data_flitwidth <= 2'h0;
  else 					cxs_tx_data_flitwidth <= (tx_cxs_activeack) ? DATA_FLITWIDTH : 2'h0;
  
assign tx_dp = (^(data_buf[255:0])); 	 //all the datfields ;
assign tx_cp = (^(tx_header[253:0])); 	//all the header fields ;

always_ff @ (posedge tx_cxs_clk or negedge tx_cxs_rst_n) 
    if (!tx_cxs_rst_n)	tx_cxs_activeack_d <= 1'b0;
	else                tx_cxs_activeack_d <= tx_cxs_activeack;

assign tx_header_err_vld = (!tx_cxs_activeack)?1'b0 : 
                           (tx_cxs_activeack_d) ? ((tx_header[253:52]!= 202'h0)	        | 
												   (tx_header[51:49] != tx_cxs_prcltype)|
												   (tx_header[48:47] != 2'h1)		    |
                                                   (tx_header[46:45] != 2'h1)		    |
												   (tx_header[44]	 != tx_cxs_last)	|
												   (tx_header[43:14] != 30'h0)		    |
                                                   (tx_header[13:0]  != tx_cxs_cntl[13:0])): 1'b0; 
												 
assign tx_header_det = (data_buf_en & (! data_buf_valid))? 1'b1:1'b0;

assign tx_header = (tx_header_det | data_buf_en) ? {
  tx_dp,					// [255]
  tx_cp,  					// [254]
...
always_ff @ (posedge tx_cxs_clk or negedge tx_cxs_rst_n)
	if (!tx_cxs_rst_n) 	{tx_header_reg_vld,tx_header_reg} <= 257'h0;
	else              	{tx_header_reg_vld,tx_header_reg} <= (tx_header_det | data_buf_en) ? {1'b1,tx_header[255:0]} :257'h0; 
...
always_ff @(posedge tx_cxs_clk or negedge tx_cxs_rst_n)
	...
	else				{tx_pkt_vld,tx_pkt_data} <= (data_buf_valid) ? {1'b1,data_buf[255:0],tx_header_reg[255:0]} : 513'h0;
...
always_ff @(posedge tx_cxs_clk or negedge tx_cxs_rst_n)
  ...
  else 					pkt_send_sts <= (tx_header_err_vld) ? 2'b10 : (data_buf_valid & (!tx_header_err_vld)) ? 2'b01 : 2'b00;
...
// ============================================================
  ...
  always_ff @(posedge tx_cxs_clk or negedge tx_cxs_rst_n)
   if (!tx_cxs_rst_n)	data_buf_en <= 1'b0;
   else 				data_buf_en <= 1'b0;
	
  always_ff @(posedge tx_cxs_clk or negedge tx_cxs_rst_n)
   if (!tx_cxs_rst_n)	cxs_tx_max_pkt_perflit <= 2'h0;
   else 				cxs_tx_max_pkt_perflit <= 2'h0;
  
  always_ff @(posedge tx_cxs_clk or negedge tx_cxs_rst_n)
   if (!tx_cxs_rst_n)	cxs_tx_data_flitwidth <= 2'h0;
   else 				cxs_tx_data_flitwidth <= 2'h0;
  ...
  always_ff @ (posedge tx_cxs_clk or negedge tx_cxs_rst_n)
	if (!tx_cxs_rst_n) 	{tx_header_reg_vld,tx_header_reg} <= 257'h0;
	else              	{tx_header_reg_vld,tx_header_reg} <= 257'h0;
